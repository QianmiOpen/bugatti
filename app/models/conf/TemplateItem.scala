 ...
 * 项目模板描述表
 */
case class TemplateItem(id: Option[Int], tid: Option[Int], itemName: String, itemDesc: Option[String], order: Int)
class TemplateItemTable(tag: Tag) extends Table[TemplateItem](tag, "template_item") {
  ...
  override def * = (id.?, tid.?, itemName, itemDesc.?, order) <> (TemplateItem.tupled, TemplateItem.unapply _)
