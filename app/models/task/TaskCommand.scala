 ...
 */
case class TaskCommand(id: Option[Int], taskId: Int, command: String, status: Int, orderNum: Int)

class TaskCommandTable(tag: Tag) extends Table[TaskCommand](tag, "task_command"){
  def id = column[Int]("id", O.PrimaryKey, O.AutoInc)
  def taskId = column[Int]("task_id", O.NotNull)
  def command = column[String]("command", O.NotNull, O.DBType("VARCHAR(100)"))
  def status = column[Int]("status", O.NotNull)
  def orderNum = column[Int]("order_num", O.NotNull)

  override def * = (id.?, taskId, command, status, orderNum) <> (TaskCommand.tupled, TaskCommand.unapply _)
}
