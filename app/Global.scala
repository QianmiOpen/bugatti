...
object AppData {
  ...
  //项目类型
  def projectTypeScript(implicit session: Session) ={
    val projectType = TableQuery[ProjectTypeTable]
    if(!MTable.getTables(projectType.baseTableRow.tableName).list.isEmpty) projectType.ddl.drop
    projectType.ddl.create

    val projectTypeSeq = Seq(
      ProjectType(1, "webapi")
    )
    projectType.insertAll(projectTypeSeq: _*)
  }
  //任务命令关联表
